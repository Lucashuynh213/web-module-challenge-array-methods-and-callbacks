...
function getWinnersByYear(data, getFinalsCB, getYearsCB, getWinnersCB) {
    const stage = getFinalsCB(data, getFinals);
...
function getAverageGoals(data) {
    const averageGoalsHome = data.reduce(function(acc, item){
        return acc + item['Home Team Goals'] + item['Away Team Goals'];
    }, 0);
    return (averageGoalsHome / data.length).toFixed(2);
 };
...
function getCountryWins(data) {
    const countryWins = data.reduce(function(acc, item){
        return acc + item["Home Team Initials"] + ["Away Team Initials"];
    }, 0)
}
